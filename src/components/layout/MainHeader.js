          ...
          <div
            ...
            <div className={classes["header-right"]} onClick={hideMenuHandler}>
              <span className={classes.links}>
               { !showOnLogin &&  <NavLink to={"/login"}className={navDataHandler} >Login</NavLink>}
                {showOnLogin &&<a href="#" style={{color:'#ff7722'}}> <FaUserCircle size={16} />Hi, {displayName} </a>}
                {showOnLogin && <NavLink to={"/register"}className={navDataHandler}>Register</NavLink>}
                {showOnLogin &&  <NavLink to={"/order-history"}className={navDataHandler}>My Orders</NavLink>}
                {showOnLogin &&
